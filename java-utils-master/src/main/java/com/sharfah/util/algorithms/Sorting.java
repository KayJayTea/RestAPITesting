...
public class Sorting {
	 ...
	 * Continue sweeping the array until it is sorted. Largest items will bubble to
	...
	/**
	 * Optimised bubble sort. All elements after the last swap are already sorted
	 ...
	 * Selection Sort.
	 * <p>
	 * Find the smallest element and move it to front (by swapping). Then find the
	 * second smallest and move it again. Continue.
	...
	/**
	 * Merge Sort. Divide the array in half, sort each one and then merge.
	...
	private static <E extends Comparable<E>> void mergeSort(final E[] arr, final int start, final int end,
			final E[] helper) {
		if (end - start < 2) {
			return;
		}
	 ...
	 * <li>Repeat recursively to the sub-array less than the pivot and to the
	 ...
	 * <p>
	 * Hoare partition scheme: Uses two indices that start at the ends of the array
	 * being partitioned, then move toward each other, until they detect an
	 * inversion. Swap the inverted elements.
	 * 
	 * @param <E>
	 * @param arr
	 * @param start
	...
	private static <E extends Comparable<E>> int partition(final E[] arr, int start, int end) {
		...
		// It's better to pick a random index, or the middle index, of the median of the
		// first, middle and last elements.
		final E pivot = arr[start + (end - start) / 2]; // could do (start + end)/2 but that could cause integer
														// overflow
